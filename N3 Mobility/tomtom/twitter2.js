...
if(str1.indexOf(str2) != -1){
   // alert(str2 + " found YES ###");
    ...
    if (hashTags[ii].indexOf(str7) != -1) {
        var t5 = hashTags[ii];
        twtLat = -29.823006;
        twtLon = 30.80275;
        var desc = "<div id='" + str7 + "' class='trpList'>" + t5 + "</div>";
        var rep5 = rep4.replace('<img src="images/tollicn.png" width="25" height="25" onclick="hideTabs()">MariannhillPlaza <br><hr size="1">', desc);
    } 
    if (hashTags[ii].indexOf(str8) != -1) {
        var t6 = hashTags[ii];
        twtLat = -28.58687;
        twtLon = 29.60895;
        var desc = "<div id='" + str8 + "' class='trpList'>" + t6 + "</div>";
        var rep6 = rep5.replace('<img src="images/tollicn.png" width="25" height="25" onclick="hideTabs()">BergvillePlaza <br><hr size="1">', desc);
    } 
...
//alert(endPoint);
    ...
    var ChaingeCleanN3 = ChaingeGet.replace('N3', '');
    var ChaingeClean1 = ChaingeCleanN3.replace(/\s/g, '');
...
//alert(twtContent);	
 ...
 //var PIDCount=0;
//alert(points);	 
 for (pointIndexC = 0; pointIndexC < pointsC.length && pointIndexC < 45600; pointIndexC++) {
...
pinType = "pins/weatherpin.png";		
 ...
 var strCheck2 = "MooiPlaza";
   ...
   var strBergvillePlaza = "BergvillePlaza";
   if (twtContent.indexOf(strBergvillePlaza) != -1) {
       twtLat = -28.58687;
       twtLon = 29.60895;
       twtLocation = 'yes';
       checkLastLocation = strBergvillePlaza;
   }
...
if(twtContent.indexOf(strCheckTrafficVolumes) != -1){
 ...
 var strCheckCrashUpdate = "#CrashUpdate";
   if(twtContent.indexOf(strCheckCrashUpdate) != -1){
       pinType = "pins/crashUpdatepin.png";
	   twtType = 'Crash';
	twtCrashUpdate();
	   }  
...
crashCounter = 1;
weatherCounter = 1;
// gb $("#incidentListTwt").html("<a href='#' onclick='getTwt()'>Refresh Tweets....<a>");	
var pointsTwt = getTweets();
var pointIndexTwt = 0;

	for (var pointIndexTwt = 0; pointIndexTwt < pointsTwt.length && pointIndexTwt < 54; pointIndexTwt++) {
					...
					if(tBody.indexOf(stringTraffic) != -1 && TraffVolumeState === "on"){
                       ...
                       var twtHead = "<div style='border-bottom:1px solid #b0aeae; text-align:left; display:block;'>" + "<img style='width:70%; height: auto;' src='images/twtListHead.png'>" + "<br>" + "</div>";
						//gb $("#incidentListTwt").prepend(listHolder);
                        //gb $("#incidentListTwt").prepend(twtHead);
						//gb listHolder = '';
						// crashCounter = 1;
//weatherCounter = 1;
                };
...
crashCounter = 1;
weatherCounter = 1;	
// gb $("#incidentListTwt").html("...");
// gb $(".tt-traffic-list-paging").html("<img src='images/twtlogo.png' style='max-width:55px;  height:auto; padding:3px;' onclick='hideTabs()'>");
// gb $(".tt-traffic-list-paging").css("margin-bottom", "50px");
// gb $(".tt-traffic-list-paging").css("text-align", "center");
...
var pointIndexTwt = 0;
	...
	for (var pointIndexTwt = 0; pointIndexTwt < pointsTwt.length && pointIndexTwt < 54; pointIndexTwt++) {
					...
					var stringCrash = "Crash"; 
					var stringObstruction = "Obstruction"; 
                    var stringWeather = "Weather";
                    var stringCrashUpdate = "CrashUpdate";
					...
					if(tBody.indexOf(stringObstruction) != -1 && CrashState  === "on"){
                        ...
                        if (tBody.indexOf(stringCrashUpdate) != -1 && CrashState === "on") {
                         //  alert("CrashUpdate found");
                            twtCategory = "Crash";
                            pinType = "pins/crashUpdatepin.png";
                           twtCrashUpdate();
                            } 
					...
					if (tBody === 'Tugela Mainline'){tomtom.Marker.DEFAULT_MARKER_OPTIONS = {
       				iconUrl: pinType, 
                        ...
                        var twtHead = "<div style='border-bottom:1px solid #b0aeae; text-align:left;  display:block;'>" + "<img style='width:70%; height: auto;' src='images/twtListHead.png'>" + "<br>" + "</div>";
						// GBD $("#incidentListTwt").prepend(listHolder);
                         // GBD $("#incidentListTwt").prepend(twtHead);
						 listHolder = '';
   // getTwtListLoad();
...
var iconBind = '';
	for (var pointIndexTwtList = 0; pointIndexTwtList < pointsTwtLis.length && pointIndexTwtList < 54; pointIndexTwtList++) {
